        ...
        <table className="min-w-full divide-y divide-gray-200">
            ...
            <tr>
              ...
              {columns.map((col) => (
                <th
                  ...
                  className={`px-3 py-3 ${col.align === 'right' ? 'text-right' : 'text-left'} text-[11px] font-medium text-gray-500 uppercase tracking-wider select-none ${col.sortable ? 'cursor-pointer' : ''}`}
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {pageRows.map((row, idx) => (
              <tr
                key={row.id}
                className={`${idx % 2 === 1 ? 'bg-gray-50' : ''} ${onRowClick ? 'cursor-pointer hover:bg-gray-50' : ''}`}
              ...
              >
                ...
                {columns.map((col) => {
                  const base = `px-3 py-3 text-sm text-gray-700 whitespace-nowrap ${col.align === 'right' ? 'text-right' : 'text-left'}`;
                  let content;
                  if (col.render) {
